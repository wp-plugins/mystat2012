...
if($cmn->getParam("myStat_debug")==1){
...
{
    global $cmn;
    if($cmn->getParam("myStat_debug")==1){$cmn->setDebug('SCRIPT END');};
};
function error_put($errno, $errstr, $errfile, $errline)
{
    ...
    if($cmn->getParam("myStat_debug")==1){$cmn->setDebug('SCRIPT PHP '.$errors.' '.$errstr.' '.$errfile.' '.$errline);};
    return true;
};
...
dbDelta($sql);
    ...
    $cmn->setPatam("myStat_version",$GLOBALS['myStat_version']);
    if($cmn->getPatam("myStat_saveday")==''){$cmn->setPatam("myStat_saveday",90);};
    if($cmn->getPatam("myStat_show_post_stat")==''){$cmn->setPatam("myStat_show_post_stat",0);};
    if($cmn->getPatam("myStat_debug")==''){$cmn->setPatam("myStat_debug",0);};
