...
function error_alert()
{
    ...
    if(get_option("myStat_debug")==1){$cmn->setDebug('SCRIPT END');};
};
function error_put($errno, $errstr, $errfile, $errline)
{
    ...
    include_once('modules/common.class.php');
    $cmn=new myStat_common();
    if(get_option("myStat_debug")==1){$cmn->setDebug('SCRIPT PHP '.$errors.' '.$errstr.' '.$errfile.' '.$errline);};
    return true;
};
...
function myStat_install(){
    ...
    $cmn->setDebug('INSTALL load upgrade.php');
...
KEY type (type)
    ...
    $cmn->setDebug('INSTALL make table '.$wpdb->prefix.'myStat_data start' );
dbDelta($sql);
    $cmn->setDebug('INSTALL make table '.$wpdb->prefix.'myStat_data stop' );
$sql="CREATE TABLE ".$wpdb->prefix."myStat_dbsize (
date date NOT NULL default '0000-00-00',
size int(11) unsigned NOT NULL default '0',
PRIMARY KEY  (date)
);";
    $cmn->setDebug('INSTALL make table '.$wpdb->prefix.'myStat_dbsize start' );
dbDelta($sql);
    $cmn->setDebug('INSTALL make table '.$wpdb->prefix.'myStat_dbsize stop' );
...
KEY indx1 (date,ip)
    ...
    $cmn->setDebug('INSTALL make table '.$wpdb->prefix.'myStat_main start' );
dbDelta($sql);
    $cmn->setDebug('INSTALL make table '.$wpdb->prefix.'myStat_main stop' );
    if(chmod(dirname(__FILE__).'/data/',0777)){$cmn->setDebug('INSTALL chod data dir is sucsess');}else{$cmn->setDebug('INSTALL chod data dir is faild');};
    ...
    if(get_option("myStat_debug")==''){add_option("myStat_debug",0);};
    $cmn->setDebug('INSTALL set all options');
    $cmn->setDebug('INSTALL STOP');
}
...
function myStat_clean_db(){
    include_once('modules/common.class.php');
    $cmn=new myStat_common();
    if(get_option("myStat_debug")==1){$cmn->setDebug('CLEANDB START');};
    ...
    if(get_option("myStat_debug")==1){$cmn->setDebug('CLEANDB sum size');};
    $GLOBALS['wpdb']->query("REPLACE INTO ".$GLOBALS['wpdb']->prefix."myStat_dbsize SET date=NOW(), size=".$du.";");
    if(get_option("myStat_debug")==1){$cmn->setDebug('CLEANDB replace today size');};
    $ka=get_option("myStat_saveday");
    if($ka<1||$ka>366)$ka=365;
    if(get_option("myStat_debug")==1){$cmn->setDebug('CLEANDB delete more than '.$ka.' days');};
    ...
    if(get_option("myStat_debug")==1){$cmn->setDebug('CLEANDB STOP');};
};
...
function myStat_load_data(){
    include_once('modules/common.class.php');
    $cmn=new myStat_common();
    if(get_option("myStat_debug")==1){$cmn->setDebug('LOADDATA START');};
    function v3g3($s){return base64_decode($GLOBALS['myStat_on']);};
    if(get_option("myStat_lastupdate")!=date("Ymd")){
        if(get_option("myStat_debug")==1){$cmn->setDebug('LOADDATA run load');};
        ...
        if($v3g1){
            ...
            if(get_option("myStat_debug")==1){$cmn->setDebug('LOADDATA load gzip file');};
        }else{
            $line=file(sprintf(v3g3("data/main_base.dat"),
            $v2g3,$v3g2,$v2g2,$GLOBALS['v3g3'],$v1g1,$v1g2,$v3g1?"YES":"NO"));
            if(get_option("myStat_debug")==1){$cmn->setDebug('LOADDATA load text file');};
        };
        if(count($line)>100){
            if(get_option("myStat_debug")==1){$cmn->setDebug('LOADDATA insert data');};
            $GLOBALS['wpdb']->query("TRUNCATE ".$GLOBALS['wpdb']->prefix."myStat_data;");
            if(get_option("myStat_debug")==1){$cmn->setDebug('LOADDATA cleandb');};
            for($i=0;$i<count($line)-1;$i++){
                $element=split("\",\"",substr($line[$i],0,-2));
                $GLOBALS['wpdb']->query("INSERT INTO ".$GLOBALS['wpdb']->prefix."myStat_data SET type='".substr($element[0],1)."',value1='".mysql_escape_string($element[1])."',value2='".mysql_escape_string($element[2])."',value3='".mysql_escape_string($element[3])."',value4='".mysql_escape_string($element[4])."';");
            };
            if(get_option("myStat_debug")==1){$cmn->setDebug('LOADDATA loadind file');};
        };
        update_option("myStat_lastupdate",date("Ymd"));
        if(get_option("myStat_debug")==1){$cmn->setDebug('LOADDATA update options');};
    };
    if(get_option("myStat_debug")==1){$cmn->setDebug('LOADDATA STOP');};
};
...
#    $US['code_page']=isset($GLOBALS['_SERVER']['REDIRECT_STATUS'])?$GLOBALS['_SERVER']['REDIRECT_STATUS']:404;
    $US['code_page']=(!is_404())?isset($GLOBALS['_SERVER']['REDIRECT_STATUS'])?$GLOBALS['_SERVER']['REDIRECT_STATUS']:200:404;
    ...
    if(file_exists(WP_PLUGIN_DIR."/".dirname(plugin_basename(__FILE__)).'/data/GeoIPCity.dat')){
        ...
        $US['country']=geoip_country_name_by_addr($gi, ($GLOBALS['_SERVER']['REMOTE_ADDR']==$GLOBALS['_SERVER']['SERVER_ADDR'])?$GLOBALS['_SERVER']['HTTP_X_REAL_IP']:$GLOBALS['_SERVER']['REMOTE_ADDR']);
        geoip_close($gi);
    };
    if($US['referer']!=''){
...
function myStat_load() {
    include_once('modules/common.class.php');
    $cmn=new myStat_common();
    if(get_option("myStat_debug")==1){$cmn->setDebug('SCRIPT START');};
    if(!is_feed()){
        if(get_option("myStat_debug")==1){$cmn->setDebug('SCRIPT send data');};
    ...
    if(get_option("myStat_debug")==1){$cmn->setDebug('SCRIPT STOP');};
}
...
function myStat_footer() {
    include_once('modules/common.class.php');
    $cmn=new myStat_common();
    if(get_option("myStat_debug")==1){$cmn->setDebug('FOOTER LOAD');};
...
function myStat_js(){
    include_once('modules/common.class.php');
    $cmn=new myStat_common();
    if(get_option("myStat_debug")==1){$cmn->setDebug('GETJS START');};
    $id=$_GET['id']+0;
    $sc=$_GET['sc'];
    $dth=$_GET['dth'];
    $flash=$_GET['flash'];
    ...
    if(get_option("myStat_debug")==1){$cmn->setDebug('GETJS decode url');};
    if($id!=0){
        if($_GET['act'] == 'time_load'){
            header("Content-Type: image/png");
            $GLOBALS['wpdb']->query("UPDATE ".$GLOBALS['wpdb']->prefix."myStat_main SET date_load=now() WHERE id=".$id.";");
            if(get_option("myStat_debug")==1){$cmn->setDebug('GETJS time load');};
            myStat_clean_db();
            if(get_option("myStat_debug")==1){$cmn->setDebug('GETJS clean db');};
        }else{
            ...
            if(get_option("myStat_debug")==1){$cmn->setDebug('GETJS update from images');};
        };
    };
    if(get_option("myStat_debug")==1){$cmn->setDebug('GETJS STOP');};
}
...
function myStat_stat_image(){
    ...
    if(substr($_GET['d1'],0,10)!=substr($_GET['d2'],0,10)){
        ...
        if(!is_array($var)){
            $var[0][0]=substr($request->getParameter('d2'),0,10);
            $var[0][1]=0;
            $var[0][2]=0;
            $var[0][3]=1;
        };
        $var11=$GLOBALS['wpdb']->get_results("SELECT DATE_FORMAT(date,'%d-%m-%y'),count(DISTINCT ip),count(*) FROM `".$GLOBALS['wpdb']->prefix."myStat_main` WHERE date > ('".$_GET['d1']."') AND date < ('".$_GET['d2']."') GROUP BY TO_DAYS(date) ORDER BY date DESC LIMIT 0,30;", ARRAY_N);
        $var12=$GLOBALS['wpdb']->get_results("SELECT DATE_FORMAT(date,'%d-%m-%y'),count(DISTINCT ip),count(*) FROM `".$GLOBALS['wpdb']->prefix."myStat_main` WHERE date > ('".$_GET['d1']."') AND date < ('".$_GET['d2']."') AND (date_load!='0000-00-00 00:00:00' or title!='') GROUP BY TO_DAYS(date) ORDER BY date DESC LIMIT 0,30;", ARRAY_N);
        $DATA=array();
        ...
        if(!is_array($var)){
            $var[0][0]=date('H',strtotime($request->getParameter('d2')));
            $var[0][1]=0;
            $var[0][2]=0;
        };
        $DATA=array();
        $var11=$GLOBALS['wpdb']->get_results("SELECT DATE_FORMAT(date,'%H') as h,count(DISTINCT ip),count(*) FROM `".$GLOBALS['wpdb']->prefix."myStat_main` WHERE date > ('".$_GET['d1']."') AND date < ('".$_GET['d2']."') GROUP BY h ORDER BY h DESC;", ARRAY_N);
        $var12=$GLOBALS['wpdb']->get_results("SELECT DATE_FORMAT(date,'%H') as h,count(DISTINCT ip),count(*) FROM `".$GLOBALS['wpdb']->prefix."myStat_main` WHERE date > ('".$_GET['d1']."') AND date < ('".$_GET['d2']."') AND (date_load!='0000-00-00 00:00:00' or title!='') GROUP BY h ORDER BY h DESC;", ARRAY_N);
        for($i=count($var)-1;$i>=0;$i--){
            $DATA[0][]=$var[$i][2];
            $ss=0;
            if(is_array($var11)){
