...
class Links_to_Pages_404{

    static function getTitle(){
        return __("Links to Pages 404","myStat");
    }

    static function getMenuItemName(){
        return array(__('Links to Pages 404','myStat'),8);
    }
    ...
    function init($date,$page=1,$exp='',$num=0){
        ...
        if($exp!=''){
            ...
            $host=$matches[3];
            $var=$cmn->getSQL("SELECT host,uri,count(DISTINCT ip) as sm FROM `%%PREFIX%%myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND code_stat=404 AND referer='".$exp."' GROUP BY uri ORDER BY sm DESC LIMIT 0,20;");
        ...
        preg_match("/(^http:\/\/)?(www\.)?.*?([^\/]+)/i",$GLOBALS['_SERVER']['HTTP_HOST'], $matches);
        $host=$matches[3];
        $var2=$cmn->getSQL("SELECT sum(count) as sm FROM `%%PREFIX%%myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND code_stat=404 AND referer!='' AND !(LOCATE('".$host."',referer)>7 AND LOCATE('".$host."',referer)<12) GROUP BY referer;");
        $all_page=count($var2);
        $var1=$cmn->getSQL("SELECT referer,sum(count) as sm FROM `%%PREFIX%%myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND code_stat=404 AND referer!='' AND !(LOCATE('".$host."',referer)>7 AND LOCATE('".$host."',referer)<12) GROUP BY referer ORDER BY sm DESC LIMIT ".($page*$limit).", ".$limit.";");
        include_once(WP_PLUGIN_DIR."/".dirname(plugin_basename(__FILE__)).'/../modules/common.class.php');
        $cmn=new myStat_common();
        $tmp=$page+1;
        echo $cmn->getPaginationLinks('javascript:myStat_loading();x_Links_to_Pages_404(\'[page_number]\',\'myStat_load\');',$all_page,$tmp,$limit);
        $j=$page*$limit+1;
        echo "<script>";
        ...
        echo "x_Links_to_Pages_404(page,id[num],num,'myStat_expand_load');";
        echo "};";
