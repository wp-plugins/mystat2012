...
class Database_Size{
    ...
    function init($date){
        $var=$GLOBALS['wpdb']->get_results("SELECT date,size,DATE_FORMAT(date,'%d') FROM `".$GLOBALS['wpdb']->prefix."myStat_dbsize` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') ORDER BY date DESC;", ARRAY_N);
