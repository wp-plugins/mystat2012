...
class Countries{
    ...
    function init($date,$page=1){
        ...
        $limit=30;$page--;
        $var2=$cmn->getSQL("SELECT count(DISTINCT ip) FROM `%%PREFIX%%myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') GROUP BY country;");
        $all_page=count($var2);
        $var1=$cmn->getSQL("SELECT country,count(DISTINCT ip) as sm FROM `%%PREFIX%%myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') GROUP BY country ORDER BY sm DESC LIMIT ".($page*$limit).", ".$limit.";");
        $tmp=$page+1;
        ...
        $var2=$cmn->getSQL("SELECT count(DISTINCT ip) FROM `%%PREFIX%%myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND country!='' GROUP BY country;");
        $all_page=count($var2);
