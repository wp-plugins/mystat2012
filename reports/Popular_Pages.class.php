...
class Popular_Pages{
    ...
    function init($date,$page=1){
        ...
        $limit=30;$page--;
        $var2=$cmn->getSQL("SELECT sum(count) as sm FROM `%%PREFIX%%myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND code_stat=200 GROUP BY uri;");
        $all_page=count($var2);
        $var1=$cmn->getSQL("SELECT host,uri,title,sum(count) as sm FROM `%%PREFIX%%myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND code_stat=200 GROUP BY uri ORDER BY sm DESC LIMIT ".($page*$limit).", ".$limit.";");
        $tmp=$page+1;
