...
class Configuration{
    ...
    function init($date,$SD='',$SPS='',$debug=''){
        ...
        if($SD!=''){
            ...
            if(get_option("myStat_debug")==1 and $debug!='true'){unlink(dirname(__FILE__).'/../data/debug.log');};
            if($debug=='true'){update_option("myStat_debug",1);}else{update_option("myStat_debug",0);};
            if(get_option("myStat_debug")==1){$cmn->setDebug('SAVE OPTIONS (report '.$this->getTitle().')');};
        };
