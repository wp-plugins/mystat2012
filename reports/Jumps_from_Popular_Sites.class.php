...
class Jumps_from_Popular_Sites{

    static function getTitle(){
        return __("Jumps from Popular Sites","myStat");
    }

    static function getMenuItemName(){
        return array(__('Jumps from Popular Sites','myStat'),5);
    }
    ...
    function init($date,$exp='',$num=0){
        ...
        if($exp!=''){
            ...
            $host=$matches[3];
            $var1=$cmn->getSQL("SELECT value3 FROM %%PREFIX%%myStat_data WHERE type='2' AND value2='".$exp."';");
            $sql='';
        ...
        $var1=$cmn->getSQL("SELECT t1.value2,count(t2.referer) as sm,referer FROM %%PREFIX%%myStat_data t1, ".$GLOBALS['wpdb']->prefix."myStat_main t2 WHERE  t2.date >= ('".$date[0]."') AND t2.date <= ('".$date[1]."') AND t2.referer!='' AND t2.code_stat=200 AND t1.type='2' AND LOCATE(value3,referer)!=0 GROUP BY t1.value2 ORDER BY sm DESC;");
        echo "<script>";
        ...
        echo "x_Jumps_from_Popular_Sites(id[num],num,'myStat_expand_load');";
        echo "};";
