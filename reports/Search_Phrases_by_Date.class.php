...
class Search_Phrases_by_Date{
    ...
    function init($date,$page=1){
        ...
        $var1=$cmn->getSQL("SELECT referer,count,www,host,uri,DATE_FORMAT(date,'%d-%m-%Y %H-%i'),DATE_FORMAT(date,'%d-%m-%Y') FROM %%PREFIX%%myStat_main WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND referer!='' AND !(LOCATE('".$host."',referer)>7 AND LOCATE('".$host."',referer)<12) ORDER BY date DESC;");
        $k=0;
