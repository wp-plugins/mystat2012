...
class Site_Usage{
    ...
    function init($date){
        ...
        $var11=$GLOBALS['wpdb']->get_results("SELECT DATE_FORMAT(date,'%d-%m-%y'),count(DISTINCT ip),count(*) FROM `".$GLOBALS['wpdb']->prefix."myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') GROUP BY TO_DAYS(date);", ARRAY_N);
        $var12=$GLOBALS['wpdb']->get_results("SELECT DATE_FORMAT(date,'%d-%m-%y'),count(DISTINCT ip),count(*) FROM `".$GLOBALS['wpdb']->prefix."myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND (date_load!='0000-00-00 00:00:00' or title!='') GROUP BY TO_DAYS(date);", ARRAY_N);
        $var13=$GLOBALS['wpdb']->get_results("SELECT DATE_FORMAT(date,'%d-%m-%y'),count(DISTINCT user),count(*) FROM `".$GLOBALS['wpdb']->prefix."myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND user!='' GROUP BY TO_DAYS(date);", ARRAY_N);
        $var14=$GLOBALS['wpdb']->get_results("SELECT DATE_FORMAT(date,'%d-%m-%y'),sum(count) FROM `".$GLOBALS['wpdb']->prefix."myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND (date_load!='0000-00-00 00:00:00' or title!='') GROUP BY TO_DAYS(date);", ARRAY_N);
