...
class Popular_Pages_404{
    ...
    function init($date,$page=1,$exp='',$num=0){
        ...
        if($exp!=''){
            ...
            $host=$matches[3];
            $var=$cmn->getSQL("SELECT referer,count(DISTINCT ip) as sm FROM `%%PREFIX%%myStat_main` WHERE uri='".$exp."' AND date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND code_stat=404 AND (LOCATE('".$host."',referer)>7 AND LOCATE('".$host."',referer)<12) GROUP BY referer ORDER BY sm DESC LIMIT 0,20;");
        ...
        $limit=30;$page--;
        $var2=$cmn->getSQL("SELECT sum(count) as sm FROM `%%PREFIX%%myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND code_stat=404 GROUP BY uri;");
        $all_page=count($var2);
        $var1=$cmn->getSQL("SELECT host,uri,title,sum(count) as sm FROM `%%PREFIX%%myStat_main` WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') AND code_stat=404 GROUP BY uri ORDER BY sm DESC LIMIT ".($page*$limit).", ".$limit.";");
        $tmp=$page+1;
