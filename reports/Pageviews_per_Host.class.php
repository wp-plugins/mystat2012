...
class Pageviews_per_Host{

    static function getTitle(){
        return __("Pageviews per Host","myStat");
    }

    static function getMenuItemName(){
        return array(__('Pageviews per Host','myStat'),2);
    }
    ...
    function init($date){
        ...
        $var1=$cmn->getSQL("SELECT sum(count) as st, ip FROM %%PREFIX%%myStat_main WHERE date >= ('".$date[0]."') AND date <= ('".$date[1]."') GROUP BY ip ORDER by st;");
        $all_sum=0;$max=0;
